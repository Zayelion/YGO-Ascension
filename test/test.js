...
describe('API', function () {
    ...
    it('#create_duel', function (done) {
        ...
        assert.property(duel, 'interpreter');
        assert.property(duel, 'game_field');
        assert.property(duel, 'mtrandom');
        ...
        assert.isObject(duel.interpreter);
        assert.isObject(duel.game_field);
        assert.isNumber(duel.mtrandom);
        done();
    });
    it('#set_player_info', function () {
        ...
        assert.isObject(duel.game_field.player_info[0]);
        assert.isNumber(duel.game_field.player_info[0].lp);
        assert.isNumber(duel.game_field.player_info[0].start_count);
        assert.isNumber(duel.game_field.player_info[0].draws_count);
    });
});